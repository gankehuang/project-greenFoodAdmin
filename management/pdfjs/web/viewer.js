...
var DEFAULT_URL = 'http://green.xgmof.net.cn/zcqapi/File' + getUrlParam('url');   //获取地址栏参数
console.log(DEFAULT_URL);

localStorage.setItem('PdfUrl', getUrlParam('url') );
      ...
      if(localStorage.getItem('pdfjs.history')){
          if(typeof JSON.parse(localStorage.getItem('pdfjs.history')) === 'object' ){
              ...
              if(history.files && history.files[0]){
                    // 跳转指定页码
                    console.log( localStorage.getItem('PDFmunePage') );
                    _this.database.files[index].page = localStorage.getItem('PDFmunePage');

                    let history = JSON.parse( localStorage.getItem('pdfjs.history') )
                    history.files[0].page = data;
